...
for f1 in "$readPath"/*_R1_001.fastq.gz; do
	...
	# No Adapter Trimming
	java -jar $EBROOTTRIMMOMATIC/trimmomatic-0.39.jar PE -threads 8 -phred"$score" $f1 $f2 $sampleTag"_pForward.fq.gz" $sampleTag"_uForward.fq.gz" $sampleTag"_pReverse.fq.gz" $sampleTag"_uReverse.fq.gz" LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36
